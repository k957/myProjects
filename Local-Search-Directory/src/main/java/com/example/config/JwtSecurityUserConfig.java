...
public class JwtSecurityUserConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests().antMatchers("**/v1/**").authenticated().and().exceptionHandling()
	 ...
	 * JedisConnectionFactory();//custom hostname and port number here }
	 * 
	 * @Bean RedisTemplate<Long, User> redisTemplate() { RedisTemplate<Long, User>
